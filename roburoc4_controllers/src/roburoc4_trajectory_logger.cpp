...
class TrajectoryLoggerNode {
	...
	private:
		...
		roburoc4_controllers::Trajectory trajectory;
...
TrajectoryLoggerNode::TrajectoryLoggerNode() {
	...
	/* Publishers */
	lineSegmentPub_ = nh_.advertise<roburoc4_controllers::LineSegment>("roburoc4_controllers/lineSegment",1);
	trajectoryPub_ = nh_.advertise<roburoc4_controllers::Trajectory>("roburoc4_controllers/trajectory",1);
	stringPub_ =  nh_.advertise<std_msgs::String>("roburoc4_printString", 10);

	/* Subscribers */
	stateSub_ = nh_.subscribe<roburoc4_state_estimator::States>("roburoc4_state_estimator/states", 1 , &TrajectoryLoggerNode::statesCallback, this);
	loggerCmdSub_ = nh_.subscribe<std_msgs::UInt8>("roburoc4_controllers/logger_cmd", 1 , &TrajectoryLoggerNode::loggerCmdCallback, this);
	
	/* Different used vars */
	roburoc4_controllers::TrajectoryPoints point;
...
void TrajectoryLoggerNode::statesCallback(const roburoc4_state_estimator::States::ConstPtr& newStates){
