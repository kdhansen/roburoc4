...
roburoc4_state_estimator::States current_state;
bool got_state = false;

void getState(const roburoc4_state_estimator::StatesConstPtr& state)
{
	if(!got_state)
	{
		current_state = *state;
...
int main(int argc, char *argv[])
{
	...
	ros::NodeHandle ph("~");
	ros::Publisher trajectory_pub = ph.advertise<roburoc4_controllers::Trajectory>("trajectory", 1);
	...
	// Fill trajectory.
	roburoc4_controllers::Trajectory circle_trajectory;
	double angle = current_state.theta;
	for (int i = 0; i < num_samples_on_circle; ++i)
	{
		roburoc4_controllers::TrajectoryPoints tp;
